# --- START OF FILE tools.py (DEFINITIVELY COMPLETE AND FINAL) ---
...
class SelectTool(Tool):
    def __init__(self, canvas):
        ...
        self.is_multiselecting=False; self.selection_rect=None; self.action_start_position=None; self.original_shapes_for_action=[]; self.dragging=False; self.scaling=False; self.rotating=False; self.scale_corner=None; self.scale_center=None; self.node_editing_active=False
    ...
    def deactivate(self):
        self.canvas.selected_shapes.clear(); self.is_multiselecting=False; self.selection_rect=None; self.action_start_position=None; self.original_shapes_for_action.clear(); self.dragging=False; self.scaling=False; self.rotating=False; self.node_editing_active=False
        self.dragged_node_info = None; self.original_node_position=None
        self.canvas.setCursor(QCursor(Qt.CursorShape.ArrowCursor)); super().deactivate()
    ...
    def mousePressEvent(self, event):
        ...
        if self.node_editing_active:
            ...
            if self.dragged_node_info:
                self.action_start_position = snapped_pos
                return
    ...
    def _handle_rotate_move(self, event):
        ...
        start_angle = math.atan2(start_vec.y(), start_vec.x()); current_angle = math.atan2(current_vec.y(), current_vec.x())
    ...
    def _handle_rotate_finish(self, event):
        if not self.scale_center: return
        start_vec = self.action_start_position - self.scale_center; current_vec = event.pos() - self.scale_center
        start_angle = math.atan2(start_vec.y(), start_vec.x()); current_angle = math.atan2(current_vec.y(), current_vec.x())
        angle_delta_rad = current_angle - start_angle
                ...
                for sub_path in shape.sub_paths:
                    for seg in sub_path:
                        if seg.node_type == PathSegment.SMOOTH:
                            painter.drawLine(transform.map(seg.handle1), transform.map(seg.anchor)); painter.drawLine(transform.map(seg.handle2), transform.map(seg.anchor))
                for sub_path in shape.sub_paths:
                    for seg in sub_path:
    ...
    def _handle_node_press(self, event):
        ...
        shape = self.canvas.selected_shapes[0]; transform, _ = self._get_transform_for_shape(shape)
        self.dragged_node_info = None
        if isinstance(shape, Path):
            for sp_idx, sub_path in enumerate(shape.sub_paths):
                for seg_idx, seg in enumerate(sub_path):
                    if seg.node_type == PathSegment.SMOOTH:
                        if (event.pos() - transform.map(seg.handle1)).manhattanLength() < 5: self.dragged_node_info = (shape, (sp_idx, seg_idx), "handle1"); self.original_node_position = seg.handle1; self.canvas.update(); return
                        if (event.pos() - transform.map(seg.handle2)).manhattanLength() < 5: self.dragged_node_info = (shape, (sp_idx, seg_idx), "handle2"); self.original_node_position = seg.handle2; self.canvas.update(); return
            for sp_idx, sub_path in enumerate(shape.sub_paths):
                for seg_idx, seg in enumerate(sub_path):
                    if (event.pos() - transform.map(seg.anchor)).manhattanLength() < 5:
                        self.dragged_node_info = (shape, (sp_idx, seg_idx), "anchor"); self.original_node_position = seg.anchor
            ...
            for i, node_pos in enumerate(shape.get_nodes()):
                if (event.pos() - transform.map(node_pos)).manhattanLength() < 5:
                    self.dragged_node_info = (shape, i, 'node'); self.original_node_position = node_pos; self.canvas.update(); return
    ...
    def _handle_node_move(self, event):
        ...
        if self.dragged_node_info:
            ...
            if isinstance(shape, Path):
                sub_path_idx, seg_idx = index; seg = shape.sub_paths[sub_path_idx][seg_idx]
                if node_type_str == "anchor":
                    offset = local_mouse_pos - seg.anchor
                    seg.anchor += offset; seg.handle1 += offset; seg.handle2 += offset
                elif node_type_str == "handle1":
                    if seg.node_type == PathSegment.SMOOTH:
                        seg.handle1 = local_mouse_pos
                        if QApplication.keyboardModifiers() != Qt.KeyboardModifier.AltModifier: seg.handle2 = seg.anchor - (seg.handle1 - seg.anchor)
                elif node_type_str == "handle2":
                    if seg.node_type == PathSegment.SMOOTH:
                        seg.handle2 = local_mouse_pos
                        if QApplication.keyboardModifiers() != Qt.KeyboardModifier.AltModifier: seg.handle1 = seg.anchor - (seg.handle2 - seg.anchor)
            else: shape.set_node_at(index, local_mouse_pos)
            self.canvas.update()
    
    def _handle_node_release(self, event):
        if self.dragged_node_info: pass
    ...
    def _update_cursor(self, pos):
        ...
        if self.node_editing_active and len(self.canvas.selected_shapes) == 1:
            shape = self.canvas.selected_shapes[0]; transform, _ = self._get_transform_for_shape(shape); on_node = False
...
# --- PenTool and all other tools are now included below ---
...
class PenTool(Tool):
    ...
    def mousePressEvent(self, event):
        ...
        if self.current_path:
            ...
            if len(current_sub_path) > 1 and (snapped_pos - current_sub_path[0].anchor).manhattanLength() < 10:
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.current_path and self.drag_start_pos and (event.buttons() & Qt.MouseButton.LeftButton):
            if not self.current_path.sub_paths[-1]: return
...
class TextTool(BaseDrawingTool):
    def create_shape(self):
        ...
        if text_rect.width() > 10 and text_rect.height() > 10:
            font = QFont(self.canvas.current_font); new_shape = Text(text_rect, "", font, self.canvas.current_pen_color, has_border=True, border_color=self.canvas.current_pen_color)
            self.canvas.start_text_editing_on_creation(new_shape); return new_shape
        return None
